...
contract PulseHex is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Pulse", "Pulse") {
        BNBRewardsFee = 4;
        liquidityFee = 2;
        burnFeeOnSell = 0; // burn on sell
        marketingFee = 2;
        marketingWallet = payable(0x31b5B830a8B02D79b1933ce7aBDe74230BA3908e);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee on transfers
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to]) {
                extraFee =(amount*burnFeeOnSell)/100;
                fees=fees+extraFee;
                extraFeeTaken = true;
            }
        	amount = amount-fees;
            ...
            if(extraFeeTaken){
              super._transfer(from, _burnAddress, extraFee);
              extraFeeTaken = false;  
            }
        }
    ...
    function setFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
    ...
    function setBurnFeeOnSell(uint256 _fee) public onlyOwner {
        require(_fee <= 2,"too high");
        burnFeeOnSell = _fee; // extra fee on sell
    }
